...
const CS = {
  ...
  analytics: {
    ...
    week: [
      ...
      { day: 'Fri', revenue: 91500, orders: 162 },
...
/* ----------------------------------------------------------------
   ...
   ---------------------------------------------------------------- */
const AUTH_REDIRECT_EXEMPT = ['login.html'];

function getUser() {
  try {
    return JSON.parse(sessionStorage.getItem('cs_user'));
  } catch (e) { return null; }
}
...
function checkAuth() {
  ...
  const isInRolesDir = pathParts.includes('roles');

  if (!user && !AUTH_REDIRECT_EXEMPT.includes(currentPage)) {
    const redirectPrefix = isInRolesDir ? '../' : '';
  ...
  if (user) {
    ...
    // Redirect logic for dashboard entry points
    const roleDashboards = {
      'Super Admin': 'roles/super-admin.html',
      'Admin': 'roles/admin.html',
      'Manager': 'index.html',
      'Staff': 'roles/staff.html',
      'Chef': 'index.html',
      'Cashier': 'index.html',
      'Student': 'roles/student.html',
      'Customer': 'index.html'
    };

    // Protection logic for restricted pages
    if (isInRolesDir) {
      const allowedRoles = {
        'super-admin.html': ['Super Admin'],
        'admin.html': ['Super Admin', 'Admin', 'Manager'],
        'staff.html': ['Super Admin', 'Admin', 'Manager', 'Staff', 'Chef', 'Cashier'],
        'student.html': ['Super Admin', 'Admin', 'Manager', 'Staff', 'Student', 'Chef', 'Cashier']
      };

      if (allowedRoles[currentPage] && !allowedRoles[currentPage].includes(role)) {
        window.location.href = '../index.html';
        return user;
      }
    }

    // Existing portal protections
    const restrictedPages = ['admin.html', 'inventory.html', 'outlets.html', 'kds.html'];
    if (restrictedPages.includes(currentPage) && role === 'Customer') {
      window.location.href = 'index.html';
      return user;
    }
  }
...
/* ----------------------------------------------------------------
   RBAC DATA INITIALIZATION
   ---------------------------------------------------------------- */
function initRBAC() {
  if (!localStorage.getItem('cs_course_limit')) {
    localStorage.setItem('cs_course_limit', '12');
  }

  const courses = JSON.parse(localStorage.getItem('cs_courses') || '[]');
  if (courses.length !== 6) {
    localStorage.setItem('cs_courses', JSON.stringify([
      { id: 1, name: 'Culinary Basics 101', students: 45, instructor: 'Chef Mario', category: 'Foundation', status: 'Active', image: 'https://images.unsplash.com/photo-1556910103-1c02745aae4d?auto=format&fit=crop&w=400&q=80' },
      { id: 2, name: 'Advanced Pastry Arts', students: 28, instructor: 'Chef Julia', category: 'Technique', status: 'Active', image: 'https://images.unsplash.com/photo-1555507036-ab1f4038808a?auto=format&fit=crop&w=400&q=80' },
      { id: 3, name: 'Molecular Gastronomy', students: 15, instructor: 'Chef Ferran', category: 'Innovation', status: 'Active', image: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?auto=format&fit=crop&w=400&q=80' },
      { id: 4, name: 'Wine Pairing Masterclass', students: 60, instructor: 'Chef Elena', category: 'Beverage', status: 'Upcoming', image: 'https://images.unsplash.com/photo-1510812431401-41d2bd2722f3?auto=format&fit=crop&w=400&q=80' },
      { id: 5, name: 'Butchery & Charcuterie', students: 22, instructor: 'Chef Hans', category: 'Technique', status: 'Active', image: 'https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=400&q=80' },
      { id: 6, name: 'Sushi & Sashimi Arts', students: 34, instructor: 'Chef Kenji', category: 'Cultural', status: 'Active', image: 'https://images.unsplash.com/photo-1579871494447-9811cf80d66c?auto=format&fit=crop&w=400&q=80' }
    ]));
  }

  if (!localStorage.getItem('cs_workshops')) {
    localStorage.setItem('cs_workshops', JSON.stringify([
      { id: 1, title: 'Live: Artisan Sourdough', date: 'Feb 28', time: '10:00 AM', chef: 'Chef Bianca', type: 'Live' },
      { id: 2, title: 'Workshop: Knife Care', date: 'Mar 02', time: '2:00 PM', chef: 'Chef Mario', type: 'Seminar' },
      { id: 3, title: 'Demo: Liquid Nitrogen', date: 'Mar 05', time: '11:00 AM', chef: 'Chef Ferran', type: 'Masterclass' }
    ]));
  }

  if (!localStorage.getItem('cs_resources')) {
    localStorage.setItem('cs_resources', JSON.stringify([
      { id: 1, title: 'HACCP Safety Guide', type: 'PDF', size: '2.4 MB', icon: '📄' },
      { id: 2, title: 'Mother Sauces Matrix', type: 'IMG', size: '8.1 MB', icon: '🖼️' },
      { id: 3, title: 'Kitchen Log Template', type: 'XLS', size: '1.2 MB', icon: '📊' }
    ]));
  }

  if (!localStorage.getItem('cs_pathway')) {
    localStorage.setItem('cs_pathway', JSON.stringify([
      { id: 1, title: 'Knife Mastery', status: 'completed', date: 'Jan 20' },
      { id: 2, title: 'Sauce Science', status: 'completed', date: 'Feb 05' },
      { id: 3, title: 'Emulsion Lab', status: 'in-progress', date: 'Today' },
      { id: 4, title: 'Sugar Art 101', status: 'locked', date: 'Next Week' }
    ]));
  }

  if (!localStorage.getItem('cs_skills')) {
    localStorage.setItem('cs_skills', JSON.stringify([
      { name: 'Spherification', trend: '+12%', type: 'Molecular' },
      { name: 'Sous Vide', trend: '+45%', type: 'Precision' },
      { name: 'Sugar Pulling', trend: '+8%', type: 'Technique' },
      { name: 'Dry Aging', trend: '+22%', type: 'Preservation' }
    ]));
  }

  if (!localStorage.getItem('cs_spotlight')) {
    localStorage.setItem('cs_spotlight', JSON.stringify({
      ...
      image: 'https://images.unsplash.com/photo-1577214495773-557ecb617011?auto=format&fit=crop&w=300&q=80'
    }));
  }

  if (!localStorage.getItem('cs_achievements')) {
    localStorage.setItem('cs_achievements', JSON.stringify([
      { id: 1, title: 'Early Bird', icon: '🌅', desc: 'Completed a lesson before 7 AM', date: '2026-02-10' },
      { id: 2, title: 'Perfect Searing', icon: '🔥', desc: 'Scored 100% on Searing Technique Quiz', date: '2026-02-15' },
      { id: 3, title: 'Mixology Apprentice', icon: '🧪', desc: 'Mastered 5 liquid density interactions', date: '2026-02-20' },
      { id: 4, title: 'Scholar', icon: '📖', desc: 'Read 20 Kitchen Leadership articles', date: 'Earned today' }
    ]));
  }

  if (!localStorage.getItem('cs_assignments')) {
    localStorage.setItem('cs_assignments', JSON.stringify([
      { id: 1, title: 'Hollandaise Emulsion Lab', course: 'Culinary Basics 101', due: 'Tomorrow, 5:00 PM', status: 'Pending' },
      { id: 2, title: 'Final Theory Exam', course: 'Vegan Nutrition Science', due: 'Feb 28, 12:00 PM', status: 'Upcoming' },
      { id: 3, title: 'Pastry Portfolio Submission', course: 'Advanced Pastry Arts', due: 'Mar 05, 11:59 PM', status: 'Draft' }
    ]));
  }

  const existingNews = JSON.parse(localStorage.getItem('cs_news') || '[]');
  if (existingNews.length !== 6) {
    localStorage.setItem('cs_news', JSON.stringify([
      { id: 1, type: 'Institute', title: 'New Campus Opening in Paris', date: '2 mins ago', excerpt: 'We are excited to announce our newest culinary lab in the heart of France.' },
      { id: 2, type: 'Scholarship', title: 'Fulbright Gastronomy Grants Open', date: '1 hour ago', excerpt: 'Apply now for full funding for International Technique exchanges.' },
      { id: 3, type: 'Announcement', title: 'System Maintenance: Mar 01', date: '5 hours ago', excerpt: 'The portal will be offline for 2 hours on Sunday for performance updates.' },
      { id: 4, type: 'Innovation', title: 'AI in Culinary Arts', date: 'Yesterday', excerpt: 'Exploring how generative models are rewriting the future of recipe development.' },
      { id: 5, type: 'Event', title: 'Global Chef Summit 2026', date: '1 day ago', excerpt: 'Register for the annual gathering of the world\'s most innovative culinary minds.' },
      { id: 6, type: 'Career', title: 'Michelin Star Internships', date: '2 days ago', excerpt: 'New partnerships signed with 3-star restaurants for the 2026 cohort.' }
    ]));
  }

  if (!localStorage.getItem('cs_polls')) {
    localStorage.setItem('cs_polls', JSON.stringify([
      { id: 1, question: 'Favorite Dessert?', options: ['Tiramisu', 'Lava Cake', 'Gelato', 'Baklava'], votes: [145, 132, 123, 85], active: true },
      { id: 2, question: 'Preferred Knife Type?', options: ['Chef Knife', 'Santoku', 'Paring Knife'], votes: [210, 180, 95], active: true }
    ]));
  }

  if (!localStorage.getItem('cs_faqs')) {
    localStorage.setItem('cs_faqs', JSON.stringify([
      { id: 1, question: 'How to register for courses?', answer: 'Navigate to your Student Dashboard and click the "Enroll" button on any available course card. You will receive a confirmation email within 24 hours.' },
      { id: 2, question: 'What is the global course limit?', answer: 'The global limit is a system-wide cap on active courses, managed by the Super Admin to maintain server stability and resource quality.' },
      { id: 3, question: 'Can I change my role?', answer: 'Roles are assigned by the Super Admin or your immediate Manager. If you require a role change, please contact your supervisor.' },
      { id: 4, question: 'Is there a mobile app?', answer: 'Culina Suite is fully responsive and can be accessed via any mobile browser. A dedicated native App is currently in development.' },
      { id: 5, question: 'How do I submit a course for review?', answer: 'Managers can create new courses. Once created, they undergo a quality check by the Super Admin before appearing as "Upcoming" or "Active" for students.' },
      { id: 6, question: 'Where can I see my certifications?', answer: 'Your completed courses and digital certifications are available under the "Learning Path" section on your Student Dashboard.' },
      { id: 7, question: 'Lost my password?', answer: 'Contact the Super Admin directly through the internal helpdesk or use the "Forgot Password" link on the login page.' },
      { id: 8, question: 'Are there group discounts?', answer: 'Yes, for groups of 5 or more students from the same organization, we offer a 20% discount on all Masterclasses.' }
    ]));
  }

  if (!localStorage.getItem('cs_comments')) {
    localStorage.setItem('cs_comments', JSON.stringify([
      { id: 1, user: 'Rahul', text: 'This platform is amazing! The molecular gastronomy course blew my mind.', status: 'approved', role: 'Student' },
      { id: 2, user: 'Priya', text: 'Can we have more time for the pastry final exam?', status: 'pending', role: 'Student' },
      { id: 3, user: 'Vikram', text: 'The new sushi course looks promising. Is there a waitlist?', status: 'approved', role: 'Student' },
      { id: 4, user: 'Amit', text: 'I faced some issues with the poll animation on mobile.', status: 'pending', role: 'Staff' },
      { id: 5, user: 'Sneha', text: 'The farm-to-table lecture was truly inspiring. Highly recommend!', status: 'approved', role: 'Student' },
      { id: 6, user: 'Karan', text: 'When is the next wine pairing class scheduled?', status: 'approved', role: 'Student' },
      { id: 7, user: 'Anjali', text: 'Spam comment 123', status: 'pending', role: 'Student' },
      { id: 8, user: 'Roberto', text: 'The UI is so much smoother now with the multi-view system!', status: 'approved', role: 'Student' },
      { id: 9, user: 'Meera', text: 'Chef Mario is the best instructor for Foundation levels.', status: 'approved', role: 'Student' },
      { id: 10, user: 'Sam', text: 'Looking for a study partner for the Butchery final.', status: 'approved', role: 'Student' }
    ]));
  }

  if (!localStorage.getItem('cs_audit_logs')) {
    localStorage.setItem('cs_audit_logs', JSON.stringify([
      { id: 1, action: 'Course Limit Updated', user: 'Super Admin', timestamp: '2026-02-24 10:15:22', details: 'Limit changed from 5 to 12' },
      { id: 2, action: 'New Manager Added', user: 'Super Admin', timestamp: '2026-02-24 11:30:10', details: 'User: admin@culina.com' },
      { id: 3, action: 'Poll Created', user: 'Manager', timestamp: '2026-02-24 12:45:00', details: 'Favorite Dessert? poll initiated' },
      { id: 4, action: 'User Deleted', user: 'Super Admin', timestamp: '2026-02-24 14:20:15', details: 'Inactive staff account removed' },
      { id: 5, action: 'System Backup', user: 'System', timestamp: '2026-02-24 15:00:00', details: 'Daily automated backup successful' },
      { id: 6, action: 'CSS Style Refresh', user: 'Super Admin', timestamp: '2026-02-24 16:10:00', details: 'Updated global color tokens for dark mode' },
      { id: 7, action: 'Bulk Enrollment', user: 'Staff', timestamp: '2026-02-24 16:45:22', details: 'Enrolled 25 students into Foundation course' }
    ]));
  }

  if (!localStorage.getItem('cs_users')) {
    localStorage.setItem('cs_users', JSON.stringify([
      { id: 1, name: 'Admin Manager', email: 'manager@culina.com', role: 'Manager', status: 'Active' },
      { id: 2, name: 'Senior Chef Hans', email: 'hans@culina.com', role: 'Staff', status: 'Active' },
      { id: 3, name: 'Rahul Sharma', email: 'rahul@student.com', role: 'Student', status: 'Online' },
      { id: 4, name: 'Priya Verma', email: 'priya@student.com', role: 'Student', status: 'Offline' },
      { id: 5, name: 'Chef Mario', email: 'mario@culina.com', role: 'Staff', status: 'Active' },
      { id: 6, name: 'Vikram Singh', email: 'vikram@student.com', role: 'Student', status: 'Online' },
      { id: 7, name: 'Elena Rossi', email: 'elena@culina.com', role: 'Manager', status: 'Active' },
      { id: 8, name: 'Gordon Ramsay', email: 'gordo@topchef.com', role: 'Manager', status: 'Away' }
    ]));
  }
}
...
const NAV_ITEMS = [
  ...
  { href: 'admin-today.html', icon: '◉', label: 'Analytics', restricted: true },
...
function buildNav() {
  ...
  const user = getUser();
  const isAdmin = user && (user.position === 'Manager' || user.position === 'Admin');
  const isStaff = user && ['Cashier', 'Chef', 'Manager', 'Admin'].includes(user.position);
...
function syncLocalData() {
  ...
  localStorage.removeItem('cs_courses');
  localStorage.removeItem('cs_achievements');
  localStorage.removeItem('cs_assignments');
  localStorage.removeItem('cs_news');
  localStorage.removeItem('cs_polls');
  localStorage.removeItem('cs_faqs');
  localStorage.removeItem('cs_comments');
  localStorage.removeItem('cs_audit_logs');
  localStorage.removeItem('cs_pathway');
  localStorage.removeItem('cs_skills');
  localStorage.removeItem('cs_spotlight');
  localStorage.removeItem('cs_workshops');
  localStorage.removeItem('cs_resources');
  localStorage.removeItem('cs_users');

  // Re-initialize with full dataset
  initRBAC();

  // UI Feedback
  showToast('Global System Sync Complete!', 'success');

  // Reload current page to refresh all components
  setTimeout(() => {
